...
-- LISTAR AS SIGLAS DAS FEDERACOES QUE SÃO DIRIGIDAS POR OUTRAS FEDERACOES.
SELECT DISTINCT F.SIGLA_FED
...
SELECT T.NOME
FROM  PARTIPA P RIGHT OUTER JOIN TECNICO T ON P.TECNICO_ID = T.ID_FIFA
...
SELECT J.FUNCAO
FROM JOGADOR J
WHERE J.ID IN (SELECT JC.JOGADOR_ID
...
-- LISTAR O NOME DOS JOGADORES QUE NASCERAM NO MESMO DIA E ATUAM NA MESMA FUNÇÃO
-- QUE O JOGADOR 'NEYMAR'.
SELECT J.NOME
FROM JOGADOR J
WHERE (J.NASCIMENTO, J.FUNCAO) IN (SELECT J2.NASCIMENTO, J2.FUNCAO
                                  FROM JOGADOR J2
...
SELECT E.NOME
FROM ESTADIO E
WHERE (E.CIDADE, E.RUA) = (SELECT E2.CIDADE, E2.RUA
                            FROM ESTADIO E2
